...
def get_hostname(address: Address) -> str:
    resolvectl_output = get_output(["resolvectl", "--json=short", "--type=PTR", "query", f"{reversed(address)}.in-addr.arpa"])
    resolvectl = json.loads(resolvectl_output) if resolvectl_output else {}
    return resolvectl.get("name", "")
...
def main():
    ...
    for net in ["vl-lan", "vl-user", "vl-iot", "vl-guest"]:
        ...
        for lease in networkctl.get("DHCPServer", {}).get("Leases", []):
            ...
            client_id = ":".join([hex(i).lstrip("0x") for i in lease.get("ClientId", [])])
            output.append({"hostname": hostname, "address": str(address), "expiration": str(datetime.now() + timedelta(microseconds=lease.get("ExpirationUSec", 0))), "client_id": client_id})
